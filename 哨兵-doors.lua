...
local NotifySoundId = "101511361468852"
local HidingTransparency = 0.5
...
function GetDistanceToPlayer(Pos)
...
local AdjustedPos = Pos and Pos +Vector3.new(0, 0, 1)
        ...
        for _, waypoint in pairs(p:GetWaypoints()) do
...
local HidingBox = Tabs.Visuals:AddLeftGroupbox('躲藏设置')
...
MinesFloor:AddToggle('AntiBridgeFall',{
Text = "防桥梁倒塌",
Default = false,
...
if part.Name == "PlayerBarrier" and part.Size.Y == 2.75 then
...
for _, v in pairs(workspace.CurrentRooms:GetDescendants()) do
if v.Name == "AntiBridgeBarrier" then
v:Destroy()
end
end
end
...
NotifyBox:AddToggle('NotifyChat',{
Text = "聊天通知",
Default = false,
Tooltip = "发送实体/密码到聊天"
})

NotifyBox:AddToggle('NotifyOxygen',{
Text = "氧气通知",
Default = false,
Tooltip = "缺氧时提示"
})

NotifyBox:AddToggle('NotifySound',{
Text = "提示音效",
Default = true,
Tooltip = "启用/禁用提示音效"
})

NotifyBox:AddTextbox("NotifySoundId", {
    Text = "自定义音效ID",
    Default = "101511361468852",
    Callback = function(Value)
        NotifySoundId = Value
    end
})

NotifyBox:AddDropdown("NotifySoundPreset", {
    Values = { "默认", "柔和", "警报", "自定义" },
    Default = 1,
    Text = "提示音效预设",
    Callback = function(Value)
        ...
        if Value ~= "自定义" then
            NotifySoundId = sounds[Value] or "101511361468852"
        end
    end
})
...
BypassEntityBox:AddToggle('Vacuum',{
Text = "防 真空房间",
Default = false,
Callback = function(Value)
for _, v in pairs(workspace.CurrentRooms:GetDescendants()) do
...
if RBXGeneral then RBXGeneral:SendAsync("Rush 已生成") end
end
end
...
local TextChatService = game:GetService("TextChatService")
local RBXGeneral = TextChatService.TextChannels.RBXGeneral
if RBXGeneral then RBXGeneral:SendAsync("Ambush 已生成") end
end
end
...
local TextChatService = game:GetService("TextChatService")
local RBXGeneral = TextChatService.TextChannels.RBXGeneral
if RBXGeneral then RBXGeneral:SendAsync("A-60 已生成") end
end
end
...
local TextChatService = game:GetService("TextChatService")
local RBXGeneral = TextChatService.TextChannels.RBXGeneral
if RBXGeneral then RBXGeneral:SendAsync("A-120 已生成") end
end
end
...
local TextChatService = game:GetService("TextChatService")
local RBXGeneral = TextChatService.TextChannels.RBXGeneral
if RBXGeneral then RBXGeneral:SendAsync("Glitch Rush 已生成") end
end
end
...
local TextChatService = game:GetService("TextChatService")
local RBXGeneral = TextChatService.TextChannels.RBXGeneral
if RBXGeneral then RBXGeneral:SendAsync("Glitch Ambush 已生成") end
end
end
...
local TextChatService = game:GetService("TextChatService")
local RBXGeneral = TextChatService.TextChannels.RBXGeneral
if RBXGeneral then RBXGeneral:SendAsync("Eyes 已生成") end
end
end
...
local TextChatService = game:GetService("TextChatService")
local RBXGeneral = TextChatService.TextChannels.RBXGeneral
if RBXGeneral then RBXGeneral:SendAsync("Blitz 已生成") end
end
end
...
local TextChatService = game:GetService("TextChatService")
local RBXGeneral = TextChatService.TextChannels.RBXGeneral
if RBXGeneral then RBXGeneral:SendAsync("Lookman 已生成") end
end
end
...
local TextChatService = game:GetService("TextChatService")
local RBXGeneral = TextChatService.TextChannels.RBXGeneral
if RBXGeneral then RBXGeneral:SendAsync("Jeff 已生成") end
end
end
end
if Toggles.Entity.Value then
...
if Key then
AddESP(Key, "电气间钥匙", Color3.new(0, 1, 1))
end
end
...
BypassBox:AddToggle('InfCrucifixOptimized',{
Text = "无限十字架(优化版)",
Risky = true,
Disabled = Floor.Value == "Rooms" and Floor.Value == "Outdoors" and true or false,
Tooltip = "基于速度检测自动使用十字架"
})
...
HidingBox:AddToggle('TranslucentHiding',{
Text = "躲藏时柜子透明",
Default = false,
Callback = function(Value)
end
...
HidingBox:AddSlider("HidingTransparency", {
    Text = "透明度",
    Default = 0.5,
    Min = 0.1,
    Max = 1,
...
Notify("抱歉你的执行器不支持删除Figure 因为执行器能力不足", 5)
...
if Toggles.TranslucentHiding.Value and LocalPlayer.Character:GetAttribute("Hiding") then
    for _, v in pairs(workspace.CurrentRooms:GetDescendants()) do
        if v:FindFirstChild("HiddenPlayer") and v.HiddenPlayer.Value == LocalPlayer.Character then
            for _, part in pairs(v:GetChildren()) do
                if part:IsA("BasePart") and part.Name ~= "PlayerCollision" and part.Name ~= "Collision" then
                    part.Transparency = HidingTransparency
                end
            end
        end
    end
...
if Toggles.InfCrucifixOptimized.Value then
    for _, Entity in pairs(workspace:GetChildren()) do
        local threshold = {
            RushMoving = 52,
            AmbushMoving = 70,
        ...
        if threshold and Entity.PrimaryPart then
            local velocity = Entity.PrimaryPart.AssemblyLinearVelocity.Magnitude
            local distance = GetDistanceToPlayer(Entity.PrimaryPart.Position)
            if velocity < threshold and distance < 100 then
                local Tool = LocalPlayer.Character:FindFirstChild("Crucifix")
                if Tool then
                    RemotesFolder.DropItem:FireServer(Tool)
                end
...
if Toggles.NotifyOxygen.Value and OxygenNotify > 2 then
OxygenNotify = 0
local oxygen = LocalPlayer.Character:GetAttribute("Oxygen")
if oxygen and oxygen < 50 and oxygen > 0 then
Notify("氧气不足: " .. oxygen .. "%", 3)
end
end

if Toggles.AutoHeartbeat.Value then
local Heartbeat = RemotesFolder:FindFirstChild("ClutchHeartbeat")
if Heartbeat then
pcall(function()
Heartbeat:FireServer(true)
end)
end
end
